...
class MoCapData:
    ...
    def create_tello_rigid_body(self):
        """Create a rigid body for Tello with initial position."""
        tello_rigid_body = {
            "name": "Tello",
            "position": [0, 180, 0],  # Initial position (X, Y, Z)
            "velocity": [0, 0, 0],    # Initial velocity (optional)
            "tracking_id": 1,         # Unique identifier
        }
        return tello_rigid_body

    def add_rigid_body(self, rigid_body):
        """Add a rigid body to the list."""
        self.rigid_body_list.append(rigid_body)
        print(f"Rigid body added: {rigid_body['name']}")

    def receive_mocap_data(self, data):
        """Receive motion capture data and process it."""
        print("Received mocap data:", data)

        if isinstance(data, dict):
            print("Available keys:", list(data.keys()))
            if 'Tello' in data:
                x, y, z = data['Tello']
                print(f"Updated Tello Position -> X: {x:.2f}, Y: {y:.2f}, Z: {z:.2f}")
            else:
                print("Error: 'Tello' key not found.")
        else:
            print("Error: Mocap data is not in dictionary format.")
